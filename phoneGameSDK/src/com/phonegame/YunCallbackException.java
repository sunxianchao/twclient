 ...
 * 接口调用异常信息类
 */
public class YunCallbackException extends RuntimeException {
    ...
    public YunCallbackException(int errorCode, int internalErr, String errorMessage) {
        super(errorMessage);
    ...
    public boolean bTokenInvalid() {
        if(this.internalErrorCode == ERRO_RET_OAUTH_SIGNATURE || this.internalErrorCode == ERRO_RET_TOKEN_CHANGED
            || this.internalErrorCode == ERRO_RET_TOKEN_OVERDUE) {
            return true;
        } else {
            return false;
        }
